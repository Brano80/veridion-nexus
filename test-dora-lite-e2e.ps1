...
$recommendations = Test-Step "Get Module Recommendations" {
    ...
    if ($doraLite) {
        ...
        Write-Host "   ⚠️  DORA Lite not in recommendations" -ForegroundColor Yellow
    }
...
"@
    ...
    if ($result -match "enabled.*true|t\s+\|\s+module_dora_lite") {
        ...
        Write-Host "   ⚠️  DORA Lite not found in company_module_configs" -ForegroundColor Yellow
...
# ============================================================================
# STEP 6: Test DORA Lite API Endpoints
# ============================================================================
Test-Step "Test DORA Lite Compliance Status API" {
    try {
...
Test-Step "Test DORA Lite Incidents API" {
    try {
...
Test-Step "Test DORA Lite Vendors API" {
    try {
...
Test-Step "Test DORA Lite SLA Monitoring API" {
    try {
...
# ============================================================================
Test-Step "Verify Module Status via API" {
    try {
...
if ($failed -eq 0) {
    Write-Host "🎉 ALL TESTS PASSED! DORA Lite integration is working end-to-end!" -ForegroundColor Green
    Write-Host ""
